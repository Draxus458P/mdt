const mdtApp = new Vue({
    ...
    methods: {
        ...
        EvidenceSearch() {
            if (this.evidence_search) {

                this.offender_results.query = this.offender_search;
                $.post('http://mdt/performEvidenceSearch', JSON.stringify({
                    query: this.evidence_search
                }));

                this.evidence_results.results = false;
                return;
            }
        },

        SubmitNewEvidence() {
            if (this.evidence_new.title && this.evidence_new.char_id && this.report_new.incident) {
                $.post('http://mdt/submitNewEvidence', JSON.stringify({
                    evidence_Num: this.evidence_new.Num,
                    evidence_Name: this.evidence_new.Name,
                    evidence_Date: this.evidence_new.Date,
                    evidence_Officer: this.evidence_new.Officer,
                    evidence_Pic: this.evidence_new.Pic,
                }));

                this.evidence_new.Num=null;
                this.evidence_new.Name="";
                this.evidence_new.Date="";
                this.evidence_new.Officer="";
                this.evidence_new.Pic="";
                // this.changePage("Search Reports");
                return;   
            }
        },

        OpenEvidenceDetails(id) {
            for (var key in this.evidence_results.results) {
                if (id == this.evidence_results.results[key].id) {

                    $.post('http://mdt/viewEvidence', JSON.stringify({
                        offender: this.evidence_results.results[key]
                    }));
        ...
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                if (mdtApp.officer.department != event.data.department) {
					...
					else if (event.data.department == 'sheriff') {
                        mdtApp.style.police = true;
                    }
                }
